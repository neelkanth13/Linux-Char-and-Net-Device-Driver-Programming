...
static int        dummy_device_open(struct inode *inode, 
		             	    struct file *file);
static int        dummy_device_release(struct inode *inode, 
		                       struct file *file);
static ssize_t    dummy_device_read(struct file *filp, 
		                    char __user *buf, 
				    size_t len,loff_t * off);
static ssize_t    dummy_device_write(struct file *filp, 
		                     const char *buf, 
				     size_t len, 
				     loff_t * off);

/* File operations to that can be performed on the dummy character device driver */
