...
static int __init dummy_driver_init(void)
{
    ...
    /*
     * 'Completion', the name itself says. When we want to notify or wake up 
     * some thread or something, when we finished some work, then we can use 
     * completion. We’ll take one situation. We want to wait for one thread for 
     * something to run. Until that time that thread has to sleep. Once that process 
     * finished, then we need to wake up that thread that is sleeping. We can do this 
     * by using completion without race conditions.
     * 
     * These completions are a synchronization mechanism which is a good method in 
     * the above situation mentioned rather than using improper locks/semaphores and 
     * busy-loops. 
     */
