...
static struct task_struct *dummy_thread;
...
static int __init dummy_driver_init(void)
{
    ...
    dummy_thread = kthread_create(thread_function, NULL, "dummy Thread");
    if (dummy_thread) {
        wake_up_process(dummy_thread);
    } else {
    ...
    dummy_thread = kthread_run(thread_function,NULL,"dummy Thread");
    if(dummy_thread) {
