...
/*
 ** This function will be called when we read the Device file. This is where the 
 *  kthread wakes up.
 */
static ssize_t dummy_read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
...
/*
 * In this source code, two places we are sending a wake_up. One from the read function 
 * and another one from the driver exit function. I’ve created one thread (wait_function) 
 * which has while(1). That thread will always wait for the event. It will sleep until it
 * gets a wake_up call. When it gets the wake_up call, it will check the condition. 
 * 
 * If the condition is,  then the wakeup came from the read function. 
 * If it is 2, then the wakeup came from an exit function. 
 *
 * If wake_up came from the read function, it will print the 
 * read count and it will again wait. If it is from the exit function, it will exit from 
 * the thread.
 */
static int __init dummy_driver_init(void)
{
