...
int thread_function1(void *pv)
{
    ...
    while (!kthread_should_stop()) {
        if (!spin_is_locked(&dummy_spinlock)) {
        ...
        pr_info("In Thread Function1: Global variable value: %lu\n", dummy_global_variable);
        spin_unlock(&dummy_spinlock);
        if (!spin_is_locked(&dummy_spinlock)) {
            pr_info("Spinlock released in Thread Function1\n");
        }
...
int thread_function2(void *pv)
{
    while (!kthread_should_stop()) {
        ...
        if (!spin_is_locked(&dummy_spinlock)) {
            pr_info("Spinlock is not locked in Thread Function2\n");
        }

        spin_lock(&dummy_spinlock);
        if (spin_is_locked(&dummy_spinlock)) {
            pr_info("Spinlock is locked in Thread Function2\n");
        }

        dummy_global_variable++;
        pr_info("In Thread Function2: Global variable value: %lu\n", dummy_global_variable);
        spin_unlock(&dummy_spinlock);
        if (!spin_is_locked(&dummy_spinlock)) {
            pr_info("Spinlock released in Thread Function2\n");
        }
