...
static DEFINE_RWLOCK(dummy_rwlock); 

//Dynamic method to initialize the read write spinlock
//rwlock_t dummy_rwlock;
...
int thread_function1(void *pv)
{
    while(!kthread_should_stop()) {  
        write_lock(&dummy_rwlock);
        dummy_global_variable++;
        write_unlock(&dummy_rwlock);
        msleep(1000);
    }
    return 0;
}
...
{
    while(!kthread_should_stop()) {
        read_lock(&dummy_rwlock);
        pr_info("In Thread Function2 : Read value %lu\n", dummy_global_variable);
        read_unlock(&dummy_rwlock);
        msleep(1000);
    }
    return 0;
}
