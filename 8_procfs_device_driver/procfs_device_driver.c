...
int32_t value = 0;
char dummy_array[20]="try_proc_array\n";
static int len = 1;
...
static ssize_t read_proc(struct file *filp, char __user *buffer, size_t length,loff_t * offset)
{
    ...
    if( copy_to_user(buffer,dummy_array,20) )
    {
...
static ssize_t write_proc(struct file *filp, const char *buff, size_t len, loff_t * off)
{
    ...
    if( copy_from_user(dummy_array,buff,len) )
    {
...
static int __init dummy_driver_init(void)
{
    ...
    /* Creating Proc entry under "/proc/dummy/" */
    proc_create("dummy_proc", 0666, parent, &proc_fops);
...
static void __exit dummy_driver_exit(void)
{
    ...
    //remove_proc_entry("dummy/dummy_proc", parent);

    /* remove complete /proc/dummy */
    proc_remove(parent);
